...
class Create extends Component {
    ...
    */
    ...
    /*handleCreateNewProduct = () => {
        ...
        this.props.passageInstance.createProduct(this.state.name, this.state.description, this.state.latitude.toString(), this.state.longitude.toString(), certificationsArray, JSON.stringify(customDataObject), {
                    ...
                    panelContent={
                        <div>
                            ...
                            {/*<FormGroup>
                                ...
                                <Input defaultValue="" type="select" name="select" id="exampleSelect"
                                       onChange={(e) => this.handleCategorySelect(e, 0)}>
                                    ...
                                    {this.state.ebayCategoryMap.rootCategoryNode ?
                                        this.state.ebayCategoryMap.rootCategoryNode.childCategoryTreeNodes.map((categoryObject, index) => {
                                            return (<option value={categoryObject.category.categoryId}
                                                            key={index}>{categoryObject.category.categoryName}</option>)
                                        })
                                        :
                                        undefined}
                                </Input>
                                {
                                    ...
                                    Object.keys(this.state.selectedCategories).map(categoryLevel => (
                                        this.state.selectedCategories[categoryLevel].childCategoryTreeNodes ?
                                            <Input defaultValue="" key={categoryLevel} type="select" name="select"
                                                   id="exampleSelect"
                                                   onChange={(e) => this.handleCategorySelect(e, categoryLevel)}>
                                                <option disabled value="" key="none">(select)</option>
                                                {
                            ...
                            {/*<FormGroup>
                                {
                                    ...
                                    Object.keys(this.state.customDataInputs).map(inputKey =>
                                        <FormGroup style={{display: "flex"}} key={inputKey}>
                                            <Input value={this.state.customDataInputs[inputKey].key}
                                            ...
                                            <Input value={this.state.customDataInputs[inputKey].value}
                                                   placeholder="Value (e.g. 'red')" style={{flex: 1}} onChange={(e) => {
                                                this.setState({
                                                    customDataInputs: {
                                                        ...this.state.customDataInputs,
                                                        [inputKey]: {
